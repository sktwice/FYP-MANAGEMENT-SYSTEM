                    ...
                    <div class="custom-border p-6 w-100">
                        <div class="pb-4">
                            <div>
                                ...
                                <p class="has-text-grey-light is-size-7">More than ${listSupervisors.size()} lecturers</p>
                            </div>
                            <div>
                                <div class="p-1" id="table">
                                    <table class="">
                                        ...
                                        <tbody>
                                            <c:forEach var="lecturer" items="${listSupervisors}">
                                                <tr>
                                                    ...
                                                    <td class="has-text-grey has-text-weight-semibold is-size-7 has-text-centered has-text-right-mobile p-4" data-label="Status Account" style="word-break:break-all;">
                                                    ...
                                                    <td class="has-text-grey has-text-weight-semibold is-size-7 has-text-centered has-text-right-mobile p-4" data-label="Action" style="word-break:break-all;">
                                                        <form action="UpdateSupervisorStatus" method="post">
                                                            <input type="hidden" name="lecturerId" value="${lecturer.lecturerId}" />
                                                            <input type="hidden" name="currentStatus" value="${lecturer.status}" />
                                                            <button type="submit" class="button is-small is-primary">
                                                                <c:choose>
                                                                    <c:when test="${lecturer.status == 'active'}">
                                                                        Deactivate
                                                                    </c:when>
                                                                    <c:otherwise>
                                                                        Activate
        ...
        <script>
            document.getElementById('searchInput').addEventListener('input', function () {
                ...
                rows.forEach(function (row) {
                    ...
                    var email = row.querySelector('[data-label="Status Account"]').textContent.toLowerCase();
